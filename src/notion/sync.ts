...
export type SyncConfig = { coursesDataSourceId: string; lessonsDataSourceId: string };
...
export type SyncDependencies = {
  client: NotionClient;
  replaceGenerated(files: GeneratedFiles, assets?: GeneratedFiles): Promise<void>;
  ingestRemoteAsset?: (input: { pageId: string; blockId: string; url: string }) => Promise<{
    publicPath: string;
    bytes: Uint8Array;
  }>;
};
...
async function localizeMarkdownImages(
  ...
  for (const [index, match] of matches.entries()) {
    ...
    assets[ingested.publicPath.replace(/^\/notion-assets\//, "")] = ingested.bytes;
...
export async function syncNotionCourses(config: SyncConfig, deps: SyncDependencies): Promise<SyncSummary> {
  ...
  const courseById = new Map(publication.courses.map(course => [course.id, course]));
  ...
  for (const course of publication.courses) {
    ...
    if (course.coverUrl && deps.ingestRemoteAsset) {
      const cover = await deps.ingestRemoteAsset({ pageId: course.id, blockId: "cover", url: course.coverUrl });
  ...
  for (const lesson of publication.lessons) {
    ...
    if (!course) throw new Error(`Published lesson ${lesson.id} references unknown course ${lesson.courseId}`);
    let markdown: string;
    try {
      markdown = await deps.client.retrievePageMarkdown(lesson.id);
    } catch (error) {
      throw new Error(`Failed to retrieve markdown for ${lesson.id}`, { cause: error });
    }
    ...
    files[`lessons/${course.slug}/${lesson.slug}.md`] = `${frontmatter(data)}${markdown.trim()}\n`;
  }
