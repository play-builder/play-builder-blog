...
const env = { CLOUDFLARE_PAGES_DEPLOY_HOOK_URL: "https://api.cloudflare.com/deploy-hooks/secret" };

describe("handlePublishRequest", () => {
  it("rejects non-POST requests", async () => {
    const response = await handlePublishRequest(new Request("https://blog.playbuilder.xyz/admin/api/publish"), env, {
      fetchImpl: fetch,
    });
  ...
  it("reports a deployment request without exposing the hook", async () => {
    const response = await handlePublishRequest(
      new Request("https://blog.playbuilder.xyz/admin/api/publish", { method: "POST" }),
      env,
      { fetchImpl: async () => new Response("upstream deployment id", { status: 201 }) }
    );
